# Delta for billing/subscription
...
### Requirement: list_active_near_period_end on ISubscriptionRepository

The system MUST extend `ISubscriptionRepository` with `list_active_near_period_end(days_ahead: int) -> list[Subscription]` for the monthly billing Celery task.
...
(Previously: without mp_preference_id and mp_subscription_id fields)
...
(Previously: without list_active_near_period_end)
...
The system MUST create a 14-day PRO trial Subscription when a new tenant registers. `RegisterUserCase` MUST call `TrialManagementService.start_trial(tenant_id)` after tenant creation. No payment is collected at registration time.

...
(Reason: `_get_current_admin_user`, `validate_admin_user()`, and `is_admin_user` are dead code — no production route references them. The `is_admin_user = Depends(_get_current_admin_user)` alias is removed entirely.)
(Migration: Any future tenant-scoped admin check MUST use `Depends(require_role(UserRole.ADMIN))` instead. Cross-tenant super-admin bypass is now handled solely by `bypass_filter = (user.role == UserRole.SUPER_ADMIN)`.)

### Requirement: UserEntity carries both role and is_admin

(Reason: The `is_admin` column is dropped; `role` is now the sole authority for both tenant-scoped access and cross-tenant bypass. `is_admin` being independent from `role` was a temporary state during RBAC migration.)
(Migration: All tests that asserted `user.is_admin` must now assert `user.role == UserRole.SUPER_ADMIN`. All tests asserting `is_admin is independent` must be removed.)

### Requirement: Column addition and CHECK constraint (from role-migration)

(Reason: This requirement was implemented by migration `a54adbe62ca3` and is already applied. The requirement is kept in the main spec for historical audit but its scenarios remain in the archive.)
(Migration: None — already applied in production.)

### Requirement: Data backfill (from role-migration)

(Reason: Same as above — backfill `is_admin → role` was done by migration `a54adbe62ca3`. The new migration handles the `is_admin=True → super_admin` override.)
(Migration: None — already applied.)

## EDGE CASES & CONSTRAINTS

| Case | Behavior | Rationale |
|------|----------|-----------|
| `is_admin=True` + `role=ADMIN` → after migration | `role='super_admin'` | is_admin wins unconditionally |
| `is_admin=True` + `role=OWNER` → after migration | `role='super_admin'` | is_admin wins unconditionally |
| `is_admin=False` + `role=OWNER` → after migration | `role='owner'` | unchanged |
| SUPER_ADMIN via PUT /users/{id}/role | HTTP 422 | Only OWNER can manage roles, and SUPER_ADMIN is excluded from valid values |
| SUPER_ADMIN in tenant isolation | bypasses all tenant filters | Same behavior as previous `is_admin=True` |
| SUPER_ADMIN + require_role(OWNER) | passes (rank 5 >= 4) | Incidental — SUPER_ADMIN is not intended for normal route access |
| JWT tokens in flight carrying is_admin | Not affected | JWT payload never carried is_admin field |
